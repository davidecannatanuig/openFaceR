...
merge_face <- function(x) { 
  ...
  for (i in 1:length(x)){ # This loop create per each dataframe a new variable with the video ID
    ...
    x[[i]] <- x[[i]] %>% # create the variable
      mutate(ID = id.name) %>%
      mutate(ID = as.factor(ID)) %>%# convert to factor to use the unsplit function later
      select(ID, everything()) # have it as first row
  }
...
continuous_summarise <- function(x, ...){
  ...
  x <- x %>%
    group_by(ID) %>%
    select(-c(timestamp, frame, success)) %>%
    select_if(is_continuous) %>%
    summarise_all(.funs = eval(parse(text = funlist))) #TODO add a is_frequency condition
...
event_summarise <- function(x, as = "count", ...){
  ...
  if (as == "ratio") {
    x <- x %>%
    summarise_if(is_frequency, .funs = ~ ratio(x = .))
    
  } else if (as == "count"){
    
    x <- x  %>%
      summarise_if(is_frequency, .funs = ~ count_events(x = ., ...))
...
time_summarise <- function(x){
  x %>%
    group_by(ID) %>%
    summarise(length = max(timestamp))
}
